...
def main():
    with open("input.txt") as f:
        # split line at half
        lines = [(line[:len(line.strip())//2].strip(), line[len(line.strip())//2:].strip()) for line in f.readlines()]

    repeated_characters = [set(first).intersection(set(second)) for first, second in lines]
